...
function viewStudentStats(id, toggle) {
	if (toggle == 1) {
		...
		document.getElementsById(id).style.height = "auto";
	} else if (toggle == 0) {
		document.getElementById(id).style.visibility = "hidden";
		document.getElementsById(id).style.height = "0px";
	}
}
...
function loadStatsPage() {
	...
	var wins_table = document.getElementById("wins");
	var losses_table = document.getElementById("losses");
	var home_team = "Buffs";
	for (var i = 2, row; row = table.rows[i]; i++) {
		var away_team = row.cells[1].innerHTML;
		var home_score = parseInt(row.cells[2].innerHTML, 10);
		var away_score = parseInt(row.cells[3].innerHTML, 10);
		alert(home_team + ": " + home_score + " " + away_team + ": " + away_score);
		if (home_score > away_score) {
			row.cells[4].innerHTML = home_team;
			wins += 1;
		} else if (home_score < away_score) {
			row.cells[4].innerHTML = away_team;
			loses += 1;
		} else {
			row.cells[4].innerHTML = "Tie";
		}
	}
	wins_table.innerHTML = wins;
	losses_table.innerHTML = loses;
}
...
function loadPlayersPage() {
	...
	for (var i = 0; i < players.length; i++) {
		...
		// alert(tag);
		tag_string = tag_string.concat(tag);
	}
	// alert(tag_string);
	dropdown.innerHTML = tag_string;
}
